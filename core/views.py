...
def index(request):
    ...
    for usernames in user_following_list:
        ...
        for a_post in feed_lists:
            comments = Comment.objects.filter(post = a_post)
            postComments.append(comments)

    feed_list = list(chain(*feed))
    feed_postcomments = list(chain(*postComments))

    return render(request, 'index.html', {'user_profile': user_profile, 'posts':feed_list, 'comments': feed_postcomments})
...
def postComment(request):
    if request.method == "POST":
        ...
        c = Comment(comment = comment, user = user, post = post)
        c.save()
